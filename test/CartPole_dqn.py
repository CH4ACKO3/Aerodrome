...
def main():
    ...
    parser.add_argument("--target_network_frequency", type=int, default=200,
        ...
        if step > args.learning_starts:
            if step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    td_target = batch["reward"] + args.gamma * (1 - batch["terminated"]) * (1 - batch["truncated"]) * target_max - 10.0 * batch["terminated"]
    ...
    fig, ax = plt.subplots()
    ax.plot(episode_length[:, 0], episode_length[:, 1])
    ax.set_xlabel("steps")
